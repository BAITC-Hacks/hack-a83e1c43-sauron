import { calculateReadiness, emptyTask, taskPayload } from './domain';
import type {
  Analysis,
  ProposalInput,
  Snapshot,
...
export const demo = {
  snapshot: async () => read(),
  analyze: async (draft_text: string): Promise<Analysis> => ({
    provider: 'mock',
    draft_text,
    questions,
    missing_fields: questions.map((q) => q.field),
  }),
  saveTask: async (input: TaskInput, id?: number) => {
    const data = read();
