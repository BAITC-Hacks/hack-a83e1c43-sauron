...
describe('Django request contract', () => {
  it('passes explicit confirmation when updating a published task', async () => {
    const fetch = vi.fn().mockResolvedValue(new Response(JSON.stringify({ id: 7 })));
    vi.stubGlobal('fetch', fetch);
    await api('live').saveTask({ ...emptyTask, title: 'Updated' }, 7, true);
    expect(JSON.parse(fetch.mock.calls[0][1].body)).toHaveProperty('confirmed', true);
  });
  it('sends current card answers and rejects unmapped AI questions', async () => {
    const fetch = vi.fn().mockResolvedValue(new Response(JSON.stringify({ questions: [{ field: 'budget', question: 'Какой бюджет?' }, { field: 'city', question: 'Какой город?' }, { field: 'other', question: 'Что ещё нужно?' }] })));
    vi.stubGlobal('fetch', fetch);
    await expect(api('live').analyze('Описание задачи бизнеса', { ...emptyTask, users: 'Кураторы курса' })).rejects.toThrow('некорректные вопросы');
    const body = JSON.parse(fetch.mock.calls[0][1].body);
    expect(body.card.users).toBe('Кураторы курса');
    expect(body.card).not.toHaveProperty('title');
  });
