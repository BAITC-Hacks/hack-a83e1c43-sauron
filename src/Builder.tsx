...
function BuilderForm({ task }: { task?: Task }) {
  const { mode, data, updateData, toast } = useApp();
  ...
  const isPublished = (data.tasks.find((item) => item.id === savedId) || task)?.status === 'published';
  ...
  async function next(event: FormEvent) {
    ...
    if (step === 0) {
      ...
      setError('');
      try {
        const result = await api(mode).analyze(form.draft_text.trim(), form);
        setAnalysis(result);
        changeStep(1);
      } catch (e) {
  ...
  async function save(publish: boolean) {
    if (busy) return;
    if (isPublished && !confirmedEdit) {
      setError('Подтвердите проверку изменений под рейтингом перед сохранением.');
      return;
    }
    if (!form.title.trim()) {
    ...
    setError('');
    try {
      const result = await api(mode).saveTask(taskPayload(form), savedId, isPublished && confirmedEdit);
      updateData((data) => ({
        ...data,
    ...
    <div className="page-width inner-page builder-page">
      ...
      <div className="builder-layout">
          ...
          <form className="builder-form" onSubmit={next}>
            ...
            {step === 1 && (
              <div className="form-stack">
                ...
                {analysis?.notice && <p className="inline-note" role="status">{analysis.notice}</p>}
        ...
        <aside className="builder-sidebar">
          ...
          {isPublished && (
            <ConfirmCheck checked={confirmedEdit} onChange={setConfirmedEdit}>
              Я проверил(а) изменения и подтверждаю обновление карточки в каталоге.
            </ConfirmCheck>
          )}
          <button
